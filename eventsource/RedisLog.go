...
// If no events are immediately available, it blocks using XREAD BLOCK until
// an event arrives or ctx is done. If ctx is already done when no events are
// immediately available, it returns with no error rather than blocking.
func (log *RedisLog) Read(ctx context.Context, cursor int64, limit int) (events []*Event, nextCursor int64, err error) {
	startID := fmt.Sprintf("%d-0", cursor+1)
	// The initial check always runs, even if ctx is already done; only the
	// decision to block afterward depends on ctx's remaining lifetime.
	messages, err := log.client.XRangeN(context.WithoutCancel(ctx), log.channel, startID, "+", int64(limit)).Result()
	if err != nil {
	...
	// Nothing available yet; block only if the caller left room to wait.
	if ctx.Err() != nil {
		return nil, cursor, nil
	}
	block := time.Duration(0) // block indefinitely, relying on ctx cancellation
	...
	if err == redis.Nil || ctx.Err() != nil {
		// Timed out or canceled with no new events.
		return nil, cursor, Timeout
	}
	if err != nil {
