...
func testLog(t *testing.T, implementation string, setup func(channel string) (eventsource.Log, func(), error)) {
	Convey("Given a "+implementation+" log", t, func() {
		...
		Convey("When events are appended and read from cursor 0", func() {
			...
			So(err, ShouldBeNil)
			events, cursor, err := log.Read(context.Background(), 0, 10)
			So(err, ShouldBeNil)
			...
			Convey("Then read should return events after the cursor", func() {
				...
				next, nextCursor, err := log.Read(context.Background(), 1, 10)
				So(err, ShouldBeNil)
		...
		Convey("When appending three events", func() {
			...
			Convey("And reading with limit 2", func() {
				events, cursor, err := log.Read(context.Background(), 0, 2)
				So(err, ShouldBeNil)
			...
			Convey("And reading from an up-to-date cursor", func() {
				noWaitCtx, cancel := context.WithCancel(context.Background())
				cancel()
				events, cursor, err := log.Read(noWaitCtx, 3, 10)
				So(err, ShouldBeNil)
		...
		Convey("When appending a mutable payload", func() {
			...
			Convey("And the original payload is mutated after append", func() {
				payload[0] = 'X'
				events, _, err := log.Read(context.Background(), 0, 10)
				So(err, ShouldBeNil)
			...
			Convey("And read results are mutated by the caller", func() {
				...
				So(err, ShouldBeNil)
				events[0].Data[0] = 'Y'
				again, _, err := log.Read(context.Background(), 0, 10)
				So(err, ShouldBeNil)
		...
		Convey("Given a blocked reader", func() {
			...
			go func() {
				...
				defer cancel()
				events, cursor, err := log.Read(ctx, 0, 10)
				errCh <- err
				cursorCh <- cursor
				resultCh <- events
			}()
